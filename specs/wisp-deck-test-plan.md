...
Wisp Deck is a terminal and tmux wrapper application with no web-facing UI. It consists of two layers: a Go TUI binary (wisp-deck-tui) built with Bubbletea that drives interactive terminal menus, and a bash orchestration layer (bin/wisp-deck, lib/*.sh) that manages process lifecycle, config files, and tmux sessions. The application has no HTML pages, HTTP server, browser UI, REST API, or documentation site. There is therefore no surface area for Playwright browser automation in the conventional sense. This test plan documents the mismatch, describes every interactive TUI screen and bash-driven flow in the application, and provides a blueprint for the functional test scenarios that the existing Go and bash test suites should cover. If a web front-end is added to Wisp Deck in the future (for example a settings dashboard served over localhost, or a GitHub Pages documentation site), this plan can be re-activated and the steps below can be directly translated into Playwright tests.
...
**Steps:**
  ...
  2. Verify that no HTTP server is started by any script or Go binary in the project
    - expect: Searching lib/*.sh, bin/wisp-deck, and wrapper.sh for 'http', 'serve', 'listen', ':8080', ':3000', 'localhost' returns no matches
    - expect: The wisp-deck-tui binary subcommands (main-menu, select-project, add-project, confirm, settings-menu, select-ai-tool, multi-select-ai-tool, select-terminal, select-branch, show-logo) all write to stdout and read from a TTY — they expose no TCP port
...
**Steps:**
  1. Launch wisp-deck-tui main-menu with a projects file containing two entries: 'my-app:/home/user/my-app' and 'web-service:/home/user/web'
    - expect: The terminal renders the Wisp Deck header with the animated ghost ASCII art
...
**Steps:**
  1. Launch wisp-deck-tui main-menu with an empty projects file
...
**Steps:**
  1. Launch wisp-deck-tui add-project
...
**Steps:**
  1. Launch wisp-deck-tui add-project, leave the name field empty, and press Enter
...
**Steps:**
  1. Launch wisp-deck-tui add-project and press Escape immediately
    - expect: The TUI exits
    - expect: JSON output: {"confirmed":false}
  2. Launch wisp-deck-tui add-project, type a partial name, then press Ctrl-C
    - expect: The TUI exits immediately
    - expect: JSON output: {"confirmed":false}
...
**Steps:**
  1. Launch wisp-deck-tui confirm 'Delete project my-app?'
...
**Steps:**
  1. Launch wisp-deck-tui confirm 'Delete project my-app?' and press 'n'
    - expect: JSON output: {"confirmed":false}
...
**Steps:**
  1. Launch wisp-deck-tui confirm 'Delete project my-app?' and press Escape
    - expect: JSON output: {"confirmed":false}
...
**Steps:**
  1. Launch wisp-deck-tui settings-menu
...
**Steps:**
  1. Launch wisp-deck-tui select-ai-tool with a mix of installed and uninstalled tools
...
**Steps:**
  1. Launch wisp-deck-tui multi-select-ai-tool
...
**Steps:**
  1. Launch wisp-deck-tui main-menu and send SIGHUP to its PID
...
**Steps:**
  1. Place an existing Ghostty config file at the target path, then run the config setup with 'merge' option
    ...
    - expect: The wisp-deck-specific lines are appended or merged without duplication
  2. Run the config setup with 'replace' option
    - expect: The existing config is replaced entirely with the wisp-deck config
