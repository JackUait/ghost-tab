# Wisp Deck Tabbed TUI Redesign — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace Wisp Deck's single cramped main-menu box with a tabbed UI (Projects · Settings · Stats), a contextual action bar, and refined retro-terminal styling — without changing the bash-facing JSON contract.
...
Add `"github.com/jackuait/wisp-deck/internal/models"` to the test imports.
...
Run: `go run ./cmd/wisp-deck-tui main-menu` (or the existing dev entry) and confirm: Tab cycles Projects/Settings/Stats, action bar updates with selection, `+ Add project` works, ghost still renders.
