...
# Load tab title watcher from lib if available
_WRAPPER_DIR="$(cd "$(dirname "$0")" && pwd)"
if [ -f "$_WRAPPER_DIR/lib/tui.sh" ]; then
  # shellcheck disable=SC1091  # Dynamic path, verified at runtime
  source "$_WRAPPER_DIR/lib/tui.sh"
fi
if [ -f "$_WRAPPER_DIR/lib/tab-title-watcher.sh" ]; then
  # shellcheck disable=SC1091  # Dynamic path, verified at runtime
  source "$_WRAPPER_DIR/lib/tab-title-watcher.sh"
fi
...
# Start tab title watcher before tmux (tmux blocks until session ends)
_ai_tool_pref="$(cat "${XDG_CONFIG_HOME:-$HOME/.config}/ghost-tab/ai-tool" 2>/dev/null | tr -d '[:space:]')"
start_tab_title_watcher "$SESSION_NAME" "${_ai_tool_pref:-claude}" "$PROJECT_NAME" "full" "$TMUX_CMD" "$GHOST_TAB_MARKER_FILE" "${XDG_CONFIG_HOME:-$HOME/.config}/ghost-tab"

"$TMUX_CMD" new-session -s "$SESSION_NAME" -e "PATH=$PATH" -e "GHOST_TAB_MARKER_FILE=$GHOST_TAB_MARKER_FILE" -c "$PROJECT_DIR" \
