...
**Goal:** Let the user keep several named Claude Code settings files and switch the active one at launch from Wisp Deck's main menu, remembering the last-used choice.

**Architecture:** A "config" is a settings JSON launched via `claude --settings <file>`. Configs live in `~/.config/wisp-deck/claude-configs/`, named in `claude-configs.list` (`name:file`), with the active filename stored in the `claude-config` pointer. `wrapper.sh` reads the pointer and exports `WISP_DECK_CLAUDE_SETTINGS`; `build_ai_launch_cmd` appends `--settings` when that env var is set. Selection is a new "Claude Config" row in the main menu's settings sub-view (mirrors the Sound cycle), shown only when the AI tool is claude. Management (add/rename/delete) is a new `claude-config-menu` reachable from the config menu.

**Tech Stack:** Bash (sourced `lib/*.sh` modules), Go + Bubbletea (`internal/tui`, `cmd/wisp-deck-tui`), Go-based bash integration tests (`test/bash/`).
...
- Config root: `${XDG_CONFIG_HOME:-$HOME/.config}/wisp-deck`.
...
- `lib/tmux-session.sh` — **modify.** `build_ai_launch_cmd` appends `--settings` from `WISP_DECK_CLAUDE_SETTINGS`.
- `wrapper.sh` — **modify.** Resolve active config path, export `WISP_DECK_CLAUDE_SETTINGS`.
...
- `cmd/wisp-deck-tui/main_menu.go` — **modify.** New flags wired to setters.
...
- `cmd/wisp-deck-tui/claude_config_menu.go` — **new.** Cobra subcommand wrapping the model, emits JSON.
...
- Tests: `test/bash/claude_configs_test.go`, `test/bash/tmux_session_settings_test.go`, `test/bash/wrapper_claude_config_test.go`, `internal/tui/mainmenu_claudeconfig_test.go`, `cmd/wisp-deck-tui/cmd_test.go` (extend).
...
- Produces: when env `WISP_DECK_CLAUDE_SETTINGS` is a non-empty path AND the resolved tool command is claude (normal or resume branch), the emitted command contains `--settings "<path>"`. No change for non-claude tools or when the env var is empty/unset.
...
git commit -m "feat(claude-config): pass --settings via WISP_DECK_CLAUDE_SETTINGS"
```
...
## Task 3: Wire `wrapper.sh` to export `WISP_DECK_CLAUDE_SETTINGS`
...
- Produces: when `SELECTED_AI_TOOL=claude` and an active config file exists, `WISP_DECK_CLAUDE_SETTINGS` holds its absolute path; otherwise it is empty/unset.
...
- Modify: `cmd/wisp-deck-tui/main_menu.go` (flags)
...
(c) Flags in `cmd/wisp-deck-tui/main_menu.go`. Add vars and flag registration:
...
git add internal/tui/mainmenu.go cmd/wisp-deck-tui/main_menu.go lib/menu-tui.sh internal/tui/mainmenu_claudeconfig_test.go
...
- Create: `cmd/wisp-deck-tui/claude_config_menu.go`
- Test: `cmd/wisp-deck-tui/cmd_test.go` (extend)
...
- Produces: subcommand `claude-config-menu --configs-list <path>` rendering a list of configs plus an "Add new config…" entry. Emits one JSON object on stdout:
  ...
  - `{"action":"quit"}`
- Pattern: mirror `internal/tui/terminal_selector.go` (model with list, Init/Update/View) and `cmd/wisp-deck-tui/select_terminal.go` (TTY opts, safe type assertion, JSON print). Add/rename use an inline `textinput` prompt; delete asks y/N inline.

- [ ] **Step 1: Write the failing test**

Follow the existing `TestRootCmd_SubcommandRegistered` pattern (cobra introspection, no execution — `cmd_test.go` has no run helper). Two tests: registration and the configs-list flag. Add to `cmd/wisp-deck-tui/cmd_test.go`:
...
Run: `go test ./cmd/wisp-deck-tui/ -run "TestClaudeConfigMenu|TestRootCmd_SubcommandRegistered" -v`
...
`cmd/wisp-deck-tui/claude_config_menu.go` — mirror `select_terminal.go`:
...
Run: `go test ./cmd/wisp-deck-tui/ -run "TestClaudeConfigMenu|TestRootCmd_SubcommandRegistered" -v && go build ./...`
Expected: PASS + clean build.

- [ ] **Step 5: Commit**

```bash
git add internal/tui/claude_config_menu.go cmd/wisp-deck-tui/claude_config_menu.go cmd/wisp-deck-tui/cmd_test.go
...
- Modify: `internal/tui/config_menu.go` + `cmd/wisp-deck-tui/config_menu.go` (add a "Manage Claude configs" entry returning `{"action":"manage-claude-configs"}`)
...
The dispatch logic is pure bash given a stubbed `wisp-deck-tui`. Test it by mocking `wisp-deck-tui claude-config-menu` to emit an `add` then a `quit`, and asserting a file + list line appear.
...
func TestConfigMenu_dispatch_add_then_quit(t *testing.T) {
	...
	// Mock wisp-deck-tui: first call returns add, second returns quit.
...
(c) Add the menu entry in `internal/tui/config_menu.go` (a "Manage Claude configs" row returning `{"action":"manage-claude-configs"}`) following the existing "manage-terminals" / "reinstall" entries; expose it through `cmd/wisp-deck-tui/config_menu.go` if it enumerates actions there. Mirror the existing entries exactly.
...
git add lib/config-tui.sh internal/tui/config_menu.go cmd/wisp-deck-tui/config_menu.go test/bash/config_tui_claude_test.go
...
- Create a config via the config menu → confirm `~/.config/wisp-deck/claude-configs/<slug>.json` + list line.
- In the main menu settings, cycle Claude Config → confirm `~/.config/wisp-deck/claude-config` updates.
...
- **Type consistency:** `ClaudeConfig{Name,File}`, `CycleClaudeConfig(direction)`, `CurrentClaudeConfigFile()`, `LoadClaudeConfigsList`, `ReadActiveClaudeConfig`, `WISP_DECK_CLAUDE_SETTINGS`, list format `name:file`, pointer = filename — used consistently across tasks.
